...
const char USER_LIBRARY_PATH_OPTION[]  = "-user-library-path"; // hidden option
                                                               // for
                                                               // qtcreator.sh
...
{
  if (Utils::HostOsInfo::isWindowsHost() && qApp) QMessageBox::information(
      nullptr,
      QLatin1String(Core::Constants::IDE_DISPLAY_NAME),
      toHtml(t));
  else qWarning("%s", qPrintable(t));
}

static void displayError(const QString& t)
{
  if (Utils::HostOsInfo::isWindowsHost() && qApp) QMessageBox::critical(nullptr,
                                                                        QLatin1String(
                                                                          Core::
                                                                          Constants
                                                                          ::
...
static void printVersion(const PluginSpec *coreplugin)
{
  ...
  str << '\n' << Core::Constants::IDE_DISPLAY_NAME << ' ' <<
    coreplugin->version() << " based on Qt " << qVersion() << "\n\n";
...
QString applicationDirPath(char *arg = nullptr)
{
  ...
  if (arg) dir = QFileInfo(QString::fromLocal8Bit(arg)).dir().absolutePath();

  if (QCoreApplication::instance()) return QApplication::applicationDirPath();
...
// taken from utils/fileutils.cpp. We cannot use utils here since that depends
// app_version.h.
...
static inline QStringList getPluginPaths()
{
  ...
  //    "$XDG_DATA_HOME/data/QtProject/qtcreator" or
  // "~/.local/share/data/QtProject/qtcreator" on Linux
  //    "~/Library/Application Support/QtProject/Qt Creator" on Mac
  QString pluginPath = QStandardPaths::writableLocation(
    QStandardPaths::GenericDataLocation);

  if (Utils::HostOsInfo::isAnyUnixHost() &&
      !Utils::HostOsInfo::isMacHost()) pluginPath += QLatin1String("/data");
  ...
  // Qt Creator X.Y.Z can load plugins from X.Y.(Z-1) etc, so add current and
  // previous
  // patch versions
  const QString minorVersion = QString::number(IDE_VERSION_MAJOR) + '.'
                               + QString::number(IDE_VERSION_MINOR) + '.';

  for (int patchVersion = IDE_VERSION_RELEASE; patchVersion >= 0;
       --patchVersion) rc.push_back(pluginPath + minorVersion +
...
static void setupInstallSettings(QString& installSettingspath)
{
  if (!installSettingspath.isEmpty() && !QFileInfo(installSettingspath).isDir()) {
    displayError(QString(
                   "-installsettingspath \"%0\" needs to be the path where a %1/%2.ini exist.").arg(
                   installSettingspath,
                   QLatin1String(
  ...
  // Check if the default install settings contain a setting for the actual
  // install settings.
  ...
  if (installSettings.contains(kInstallSettingsKey)) {
    QString installSettingsPath =
      installSettings.value(kInstallSettingsKey).toString();

    if (QDir::isRelativePath(installSettingsPath)) installSettingsPath =
        applicationDirPath() + '/' + installSettingsPath;
    QSettings::setPath(QSettings::IniFormat,
                       QSettings::SystemScope,
                       installSettingsPath);
  }
...
static inline QSettings* userSettings()
{
  ...
  const QString fromVariant = QLatin1String(
    Core::Constants::IDE_COPY_SETTINGS_FROM_VARIANT_STR);

  if (fromVariant.isEmpty()) return settings;
  ...
  if (!destDir.exists()) destDir.mkpath(pathFi.absolutePath());

  QDir srcDir = destDir;
  srcDir.cdUp();

  if (!srcDir.cd(fromVariant)) return settings;
  ...
  foreach(const QString& file, entries) {
    ...
    if (lowerFile.startsWith(QLatin1String("profiles.xml"))
        ...
        || lowerFile.startsWith(QLatin1String(Core::Constants::IDE_ID) +
                                ".")) QFile::copy(srcDir.absoluteFilePath(file),
                                                  destDir.absoluteFilePath(file));

    if (file == QLatin1String(Core::Constants::IDE_ID)) copyRecursively(srcDir.absoluteFilePath(
                                                                          file),
                                                                        destDir.absoluteFilePath(
                                                                          file));
  }
...
{
  if (Utils::HostOsInfo().isMacHost()) return;

  std::unique_ptr<QSettings> settings(createUserSettings());

  const bool defaultValue         = Utils::HostOsInfo().isWindowsHost();
  const bool enableHighDpiScaling = settings->value("Core/EnableHighDpiScaling",
                                                    defaultValue).toBool();

  static const char ENV_VAR_QT_DEVICE_PIXEL_RATIO[] = "QT_DEVICE_PIXEL_RATIO";

  if (enableHighDpiScaling
      && !qEnvironmentVariableIsSet(ENV_VAR_QT_DEVICE_PIXEL_RATIO) // legacy in
                                                                   // 5.6, but
                                                                   // still
                                                                   // functional
...
void loadFonts()
{
  ...
  foreach(const QFileInfo& fileInfo,
          dir.entryInfoList(QStringList("*.ttf"), QDir::Files))
...
Options parseCommandLine(int argc, char *argv[])
{
  ...
  while (it != end) {
    ...
    } else if ((arg == TEMPORARY_CLEAN_SETTINGS1) ||
               (arg == TEMPORARY_CLEAN_SETTINGS2)) {
      options.wantsCleanSettings = true;
    } else { // arguments that are still passed on to the application
      if (arg == TEST_OPTION) options.hasTestOption = true;
...
{
  Utils::Environment::systemEnvironment(); // cache system environment before we
                                           // do any changes
  ...
  if (options.userLibraryPath) {
    if ((*options.userLibraryPath).isEmpty()) {
      ...
      Utils::Environment::modifySystemEnvironment(
        { { "LD_LIBRARY_PATH", *options.userLibraryPath,
  ...
  if (!qEnvironmentVariableIsSet("QT_OPENGL")) QCoreApplication::setAttribute(
      Qt::AA_UseOpenGLES);
  ...
  if (options.settingsPath.isEmpty() &&
      (options.hasTestOption || options.wantsCleanSettings)) {
    temporaryCleanSettingsDir.reset(new Utils::TemporaryDirectory(
                                      "qtc-test-settings"));

    if (!temporaryCleanSettingsDir->isValid()) return 1;

    options.settingsPath = temporaryCleanSettingsDir->path();
  }

  if (!options.settingsPath.isEmpty()) QSettings::setPath(QSettings::IniFormat,
                                                          QSettings::UserScope,
  ...
  QString overrideLanguage =
    settings->value(QLatin1String("General/OverrideLanguage")).toString();

  if (!overrideLanguage.isEmpty()) uiLanguages.prepend(overrideLanguage);
  const QString& creatorTrPath = resourcePath() + "/translations";
  foreach(QString locale, uiLanguages) {
    locale = QLocale(locale).name();

    if (translator.load("qtcreator_" + locale, creatorTrPath)) {
      const QString& qtTrPath = QLibraryInfo::location(
        QLibraryInfo::TranslationsPath);
      const QString& qtTrFile = QLatin1String("qt_") + locale;

      // Binary installer puts Qt tr files into creatorTrPath
      if (qtTranslator.load(qtTrFile,
                            qtTrPath) || qtTranslator.load(qtTrFile,
                                                           creatorTrPath)) {
  ...
  if (!coreplugin) {
    QString nativePaths =
      QDir::toNativeSeparators(pluginPaths.join(QLatin1Char(',')));
                          ...
                          || foundAppOptions.contains(QLatin1String(CLIENT_OPTION))))
  {
    ...
    if (app.sendMessage(PluginManager::serializedArguments(), 5000 /*timeout*/,
                        pid)) return 0;
    ...
    if (app.isRunning(pid)) {
      ...
      while (button == QMessageBox::Retry) {
        ...
        if (!app.isRunning(pid)) // App quit while we were trying so start a new
                                 // creator
          button = QMessageBox::Yes;
        else button = askMsgSendFailed();
      }

      if (button == QMessageBox::No) return -1;
    }
  }
