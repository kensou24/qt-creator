...
{
  for (const QString& arg : inArgs) addArg(arg, osType);
}
...
bool FileUtils::removeRecursively(const FilePath& filePath, QString *error)
{
  ...
  if (!fileInfo.exists() && !fileInfo.isSymLink()) return true;

  QFile::setPermissions(filePath.toString(),
                        fileInfo.permissions() | QFile::WriteUser);
  ...
  if (fileInfo.isDir()) {
    ...
    QStringList fileNames = dir.entryList(QDir::Files | QDir::Hidden
                                          | QDir::System | QDir::Dirs |
                                          QDir::NoDotAndDotDot);
    foreach(const QString& fileName,
            fileNames) {
      if (!removeRecursively(filePath.pathAppended(fileName),
                             error)) return false;
    }
...
/*!
   Copies the directory specified by \a srcFilePath recursively to \a
      tgtFilePath. \a tgtFilePath will contain
...
bool FileUtils::copyRecursively(const FilePath& srcFilePath,
                                ...
                                QString *error,
                                const std::function<bool(QFileInfo, QFileInfo,
                                                         QString *)>& copyHelper)
{
  ...
  if (srcFileInfo.isDir()) {
    ...
    foreach(const QString& fileName, fileNames) {
      ...
      if (!copyRecursively(newSrcFilePath, newTgtFilePath, error,
                           copyHelper)) return false;
    }
  } else {
    if (copyHelper) {
      if (!copyHelper(srcFileInfo, tgtFilePath.toFileInfo(), error)) return false;
    } else {
...
/*!
   If this is a directory, the function will recursively check all files and
      return
   true if one of them is newer than \a timeStamp. If this is a single file,
      true will
...
bool FilePath::isNewerThan(const QDateTime& timeStamp) const
{
  ...
  if (!fileInfo.exists() || (fileInfo.lastModified() >= timeStamp)) return true;
  ...
  if (fileInfo.isDir()) {
    ...
    for (const QString& curFileName : dirContents) {
      if (pathAppended(curFileName).isNewerThan(timeStamp)) return true;
    }
  }
  return false;
}
...
/*!
   ...
   Unlike QFileInfo::canonicalFilePath(), this function will still return the
      expected deepest
...
FilePath FileUtils::resolveSymlinks(const FilePath& path)
{
  ...
  while (links-- && f.isSymLink()) f.setFile(f.dir(), f.symLinkTarget());

  if (links <= 0) return FilePath();
...
/*!
   Like QDir::toNativeSeparators(), but use prefix '~' instead of $HOME on unix
      systems when an
...
QString FileUtils::fileSystemFriendlyName(const QString& name)
{
  ...
  result.replace(QRegExp(QLatin1String("_+")),  QLatin1String("_")); // compact
                                                                     // _
  result.remove(QRegExp(QLatin1String("^_*")));                      // remove
                                                                     // leading
                                                                     // _
  result.remove(QRegExp(QLatin1String("_+$")));                      // remove
                                                                     // trailing
  ...
  if (result.isEmpty()) result = QLatin1String("unknown");
  return result;
}
...
// makes sure that capitalization of directories is canonical on Windows and OS
// X.
...
QString FileUtils::normalizePathName(const QString& name)
{
  ...
  const auto    nameC = reinterpret_cast<LPCTSTR>(nativeSeparatorName.utf16()); //
                                                                                // MinGW
  ...
  ILFree(file);
  return success ? QDir::fromNativeSeparators(QString::fromUtf16(reinterpret_cast<
                                                                   const ushort *>(
                                                                   buffer)))
         : name;
  ...
  if (HostOsInfo::isWindowsHost()) {
    ...
    if ((path.length() >= 3) && (path.at(1) == QLatin1Char(':')) &&
        ...
        && ((path.at(2) == QLatin1Char('/')) ||
            (path.at(2) == QLatin1Char('\\')))) return false;
  }
  return true;
}
...
QString FileUtils::resolvePath(const QString& baseDir, const QString& fileName)
{
  ...
  if (isAbsolutePath(fileName)) return QDir::cleanPath(fileName);

  return QDir::cleanPath(baseDir + QLatin1Char('/') + fileName);
}

FilePath FileUtils::commonPath(const FilePath& oldCommonPath,
                               const FilePath& filePath)
{
  FilePath newCommonPath = oldCommonPath;

  while (!newCommonPath.isEmpty() &&
         !filePath.isChildOf(newCommonPath)) newCommonPath =
      newCommonPath.parentDir();
...
static QByteArray fileIdWin8(HANDLE handle)
{
  ...
  if (GetFileInformationByHandleEx(handle,
    ...
    result +=
      QByteArray(reinterpret_cast<const char *>(&infoEx.FileId),
...
QByteArray FileUtils::fileId(const FilePath& fileName)
{
  ...
  if (Q_UNLIKELY(fileName.isEmpty())) return result;

  QT_STATBUF statResult;

  if (QT_STAT(fileName.toString().toLocal8Bit().constData(),
              &statResult)) return result;
...
QByteArray FileReader::fetchQrc(const QString& fileName)
{
  ...
  QTC_ASSERT(ok,
             qWarning() << fileName << "not there!"; return QByteArray());
  return file.readAll();
}
...
bool FileReader::fetch(const QString     & fileName,
                       QIODevice::OpenMode mode,
                       QString            *errorString)
{
  if (fetch(fileName, mode)) return true;

  if (errorString) *errorString = m_errorString;
  return false;
}
                       ...
                       QWidget            *parent)
{
  ...
  if (parent) QMessageBox::critical(parent, tr("File Error"), m_errorString);
  return false;
}
...
FileSaver::FileSaver(const QString& filename, QIODevice::OpenMode mode)
{
  ...
  if (HostOsInfo::isWindowsHost()) {
    ...
    for (const QString& rn : reservedNames) {
      if (fn == rn) {
        m_errorString =
          tr("%1: Is a reserved filename on Windows. Cannot save.").arg(filename);
        m_hasError = true;
        return;
      }
    }
  ...
  if (!m_file->open(QIODevice::WriteOnly | mode)) {
    QString err = QFile::exists(filename) ?
                  tr("Cannot overwrite file %1: %2") : tr(
      "Cannot create file %1: %2");
    m_errorString = err.arg(QDir::toNativeSeparators(filename),
                            m_file->errorString());
    m_hasError = true;
  }
}
...
{
  if (!m_isSafe) return FileSaverBase::finalize();

  auto sf = static_cast<SaveFile *>(m_file.get());

  if (m_hasError) {
    if (sf->isOpen()) sf->rollback();
  } else {
    setResult(sf->commit());
  }
  m_file.reset();
...
TempFileSaver::TempFileSaver(const QString& templ)
{
  ...
  if (!templ.isEmpty()) tempFile->setFileTemplate(templ);
  tempFile->setAutoRemove(false);

  if (!tempFile->open()) {
    m_errorString = tr("Cannot create temporary file in %1: %2").arg(
      QDir::toNativeSeparators(QFileInfo(
                                 tempFile->fileTemplate()).absolutePath()),
...
TempFileSaver::~TempFileSaver()
{
  ...
  if (m_autoRemove) QFile::remove(m_fileName);
}
...
{
  if (m_url.isEmpty()) return QDir::toNativeSeparators(toString());
...
{
  if (pathComponents < 0) return m_data;

  const QChar slash = QLatin1Char('/');
  int i             = m_data.lastIndexOf(slash);

  if ((pathComponents == 0) || (i == -1)) return m_data.mid(i + 1);
  ...
  if ((i > 0) &&
      (m_data.lastIndexOf(slash, i) != -1)) return m_data.mid(component);
...
FilePath FilePath::parentDir() const
{
  ...
  if (basePath.isEmpty()) return FilePath();

  const QDir base(basePath);

  if (base.isRoot()) return FilePath();
...
FilePath FilePath::fromStringWithExtension(const QString& filepath,
                                           const QString& defaultExtension)
{
  if (filepath.isEmpty() ||
      defaultExtension.isEmpty()) return FilePath::fromString(filepath);
  ...
  if (!fi.fileName().contains(dot)) {
    if (!defaultExtension.startsWith(dot)) rc += dot;
    rc += defaultExtension;
  }
...
FilePath FilePath::fromUserInput(const QString& filePath)
{
  ...
  if (clean.startsWith(QLatin1String("~/"))) clean = QDir::homePath() + clean.mid(
      1);
...
{
  if (variant.type() == QVariant::Url) return FilePath::fromUrl(variant.toUrl());
...
{
  if (!m_url.isEmpty()) return m_url == other.m_url;

  return QString::compare(m_data,
                          other.m_data,
                          HostOsInfo::fileNameCaseSensitivity()) == 0;
}
...
bool FilePath::isChildOf(const FilePath& s) const
{
  ...
  if (!m_data.startsWith(s.m_data,
                         HostOsInfo::fileNameCaseSensitivity())) return false;

  if (m_data.size() <= s.m_data.size()) return false;

  // s is root, '/' was already tested in startsWith
  if (s.m_data.endsWith(QLatin1Char('/'))) return true;

  // s is a directory, next character should be '/' (/tmpdir is NOT a child of
  // /tmp)
...
/// \returns the relativeChildPath of FilePath to parent if FilePath is a child
// of parent
...
{
  if (!isChildOf(parent)) return FilePath();
...
FilePath FilePath::pathAppended(const QString& str) const
{
  ...
  if (!isEmpty() && !m_data.endsWith(QLatin1Char('/'))) fn.m_data.append('/');
...
uint FilePath::hash(uint seed) const
{
  if (Utils::HostOsInfo::fileNameCaseSensitivity() ==
      Qt::CaseInsensitive) return qHash(m_data.toUpper(), seed);
