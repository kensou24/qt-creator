...
bool BuildableHelperLibrary::isQtChooser(const QFileInfo& info)
{
  return info.isSymLink() &&
         info.symLinkTarget().endsWith(QLatin1String("/qtchooser"));
}

/**
 * @brief 根据qtchooser目录找到qt工作目录
 * @param path
 * @return
 */
QString BuildableHelperLibrary::qtChooserToQmakePath(const QString& path)
{
  ...
  proc.setTimeoutS(1);
  SynchronousProcessResponse response = proc.runBlocking({ path,
                                                           { "-print-env" } });

  if (response.result != SynchronousProcessResponse::Finished) return QString();
...
static bool isQmake(const QString& path)
{
  ...
  /** symLinkTarget 返回连接指向的文件*/
  if (BuildableHelperLibrary::isQtChooser(fi)) fi.setFile(BuildableHelperLibrary::qtChooserToQmakePath(
                                                            fi.symLinkTarget()));
...
{
  if (path.isEmpty()) return FilePath();
  ...
  if (dir.exists(qmake)) {
    ...
    if (isQmake(qmakePath)) return FilePath::fromString(qmakePath);
  }
  ...
  for (const QFileInfo& fi : candidates) {
    if (fi.fileName() == qmake) continue;

    if (isQmake(fi.absoluteFilePath())) return FilePath::fromFileInfo(fi);
  }
  return FilePath();
}
...
FilePathList BuildableHelperLibrary::findQtsInEnvironment(const Environment& env,
                                                          int                maxCount)
{
  ...
  for (const FilePath& path : paths) {
    ...
    if (qmake.isEmpty()) continue;
    qmakeList << qmake;

    if ((maxCount != -1) && (qmakeList.size() == maxCount)) break;
  }
  return qmakeList;
}
...
{
  if (qmakePath.isEmpty()) return QString();

  //同步程序的执行，获取执行结果，SynchronousProcess
  SynchronousProcess qmake;
  qmake.setTimeoutS(5);
  SynchronousProcessResponse response = qmake.runBlocking({ qmakePath,
                                                            { "--version" } });
  ...
  static QRegExp regexp(QLatin1String(
                          "(QMake version|QMake version:)[\\s]*([\\d.]*)"),
                        Qt::CaseInsensitive);
...
QStringList BuildableHelperLibrary::possibleQMakeCommands()
{
  ...
  // On OS X, Qt 4 binary packages also has renamed qmake. There are also
  // symbolic links that are
  // named "qmake", but the file dialog always checks against resolved links
  // (native Cocoa issue)
                                       ...
                                       QString           *errorMessage)
{
  ...
  if (!FileUtils::removeRecursively(FilePath::fromString(targetDirectory),
                                    errorMessage)) return false;

  if (!QDir().mkpath(targetDirectory)) {
    *errorMessage = QCoreApplication::translate(
      "ProjectExplorer::DebuggingHelperLibrary",
      "The target directory %1 could not be created.").arg(targetDirectory);
    return false;
  }
  foreach(const QString& file, files) {
    ...
    if (destInfo.exists()) {
      if (destInfo.lastModified() >= QFileInfo(source).lastModified()) continue;

      if (!QFile::remove(dest)) {
        *errorMessage = QCoreApplication::translate(
          "ProjectExplorer::DebuggingHelperLibrary",
          "The existing file %1 could not be removed.").arg(
          destInfo.absoluteFilePath());
        return false;
      }
    }

    if (!destInfo.dir().exists()) QDir().mkpath(destInfo.dir().absolutePath());

    if (!QFile::copy(source, dest)) {
      *errorMessage = QCoreApplication::translate(
        "ProjectExplorer::DebuggingHelperLibrary",
        "The file %1 could not be copied to %2.").arg(source, dest);
      return false;
    }
  }
  return true;
                                    ...
                                    QString *output, QString *errorMessage)
{
  ...
  if (!SynchronousProcess::readDataFromProcess(proc, timeoutS, &stdOut, &stdErr,
                                               false)) {
    *errorMessage = QCoreApplication::translate(
      "ProjectExplorer::BuildableHelperLibrary",
      "Timeout after %1 s.").
...
static bool runBuildProcess(QProcess& proc,
                            ...
                            QString *output, QString *errorMessage)
{
  const bool rc = runBuildProcessI(proc,
                                   binary,
                                   args,
                                   timeoutS,
                                   ignoreNonNullExitCode,
                                   output,
                                   errorMessage);
                                         ...
                                         QString *log, QString *errorMessage)
{
  ...
  log->append(QCoreApplication::translate(
                "ProjectExplorer::BuildableHelperLibrary",
                "Building helper \"%1\" in %2\n").arg(
                arguments.helperName,
                arguments
                .directory));
  log->append(newline);

  const FilePath makeFullPath = arguments.environment.searchInPath(
    arguments.makeCommand);
  ...
  if (QFileInfo::exists(arguments.directory + QLatin1String("/Makefile"))) {
    ...
    if (!runBuildProcess(proc, makeFullPath, QStringList(cleanTarget), 30, true,
                         log, errorMessage)) return false;
  }
  QStringList qmakeArgs;

  if (!arguments.targetMode.isEmpty()) qmakeArgs << arguments.targetMode;

  if (!arguments.mkspec.isEmpty()) qmakeArgs << QLatin1String("-spec") <<
      arguments.mkspec.toUserOutput();
  ...
  log->append(QCoreApplication::translate(
                "ProjectExplorer::BuildableHelperLibrary",
                "Running %1 %2 ...\n").arg(arguments.
                                           qmakeCommand.toUserOutput(),
                                           qmakeArgs.
                                           join(QLatin1Char(' '))));

  if (!runBuildProcess(proc, arguments.qmakeCommand, qmakeArgs, 30, false, log,
                       errorMessage)) return false;
  ...
  log->append(QCoreApplication::translate(
                "ProjectExplorer::BuildableHelperLibrary",
                "Running %1 %2 ...\n")
              .arg(makeFullPath.toUserOutput(),
                   arguments.makeArguments.join(QLatin1Char(' '))));

  if (!runBuildProcess(proc, makeFullPath, arguments.makeArguments, 120, false,
                       log, errorMessage)) return false;

  return true;
}

bool BuildableHelperLibrary::getHelperFileInfoFor(
                                                    ...
                                                    bool               acceptOutdatedHelper)
{
  ...
  if (!acceptOutdatedHelper) {
    foreach(const QString& sourceFileName, sourceFileNames) {
      const QDateTime fileModified =
        QFileInfo(sourcePath + sourceFileName).lastModified();

      if (fileModified.isValid() &&
          (!sourcesModified.isValid() ||
           (fileModified > sourcesModified))) sourcesModified = fileModified;
    }
  }
  ...
  if (sourcesModified.isValid()) sourcesModified = sourcesModified.addSecs(-300);

  // look for the newest helper library in the different locations
  QString   newestHelper;
  QDateTime newestHelperModified = sourcesModified; // prevent using one that's
