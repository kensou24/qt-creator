...
/*!
    ...
    plugin \c {compatibility version <= dependency version <= plugin version} is
       searched for.

    See also ExtensionSystem::IPlugin for more information about plugin
       dependencies and
    version matching.
 */
...
/*!
    ...
    \brief The PluginSpec class contains the information of the plugin's
       embedded meta data
...
/*!
    ...
    \value  Resolved
            ...
            successfully found, and are available via the dependencySpecs()
               function.
    \value  Loaded
    ...
    \value Stopped
            The plugin has been shut down, i.e. the plugin's
               IPlugin::aboutToShutdown() function has been called.
    \value Deleted
...
/*!
    The plugin version. This is valid after the PluginSpec::Read state is
       reached.
 */
...
/*!
    The plugin compatibility version. This is valid after the PluginSpec::Read
       state is reached.
 */
...
/*!
    The plugin vendor. This is valid after the PluginSpec::Read state is
       reached.
 */
...
/*!
    The plugin copyright. This is valid after the PluginSpec::Read state is
       reached.
 */
...
/*!
    The plugin license. This is valid after the PluginSpec::Read state is
       reached.
 */
...
/*!
    The plugin description. This is valid after the PluginSpec::Read state is
       reached.
 */
...
/*!
    The category that the plugin belongs to. Categories are groups of plugins
       which allow for keeping them together in the UI.
...
/*!
    A QRegExp matching the platforms this plugin works on. An empty pattern
       implies all platforms.
    \since 3.0
 */
...
/*!
    ...
    \note This function might return false even if the plugin is loaded as a
...
{
  if (!isAvailableForHostPlatform()) return false;

  if (isForceEnabled() || isEnabledIndirectly()) return true;
...
/*!
    Returns whether the plugin is enabled via the -load option on the command
       line.
 */
...
/*!
    Returns whether the plugin is disabled via the -noload option on the command
       line.
 */
...
/*!
    The plugin dependencies. This is valid after the PluginSpec::Read state is
       reached.
 */
...
/*!
    Returns a list of descriptions of command line arguments the plugin
       processes.
 */
...
/*!
    The absolute path to the directory containing the plugin XML description
       file
...
/*!
    The absolute path to the plugin XML description file (including the file
       name)
    this PluginSpec corresponds to.
 */
...
{
  return Utils::anyOf(d->dependencySpecs.keys(),
...
bool PluginSpecPrivate::read(const QString& fileName)
{
  ...
  // 读取plugin的meta数据项，由各个插件.json文件定义
  if (!readMetaData(loader.metaData())) return false;
...
void PluginSpecPrivate::setForceEnabled(bool value)
{
  ...
  if (value) forceDisabled = false;
}

void PluginSpecPrivate::setForceDisabled(bool value)
{
  if (value) forceEnabled = false;
  forceDisabled = value;
}
...
{
  return QCoreApplication::translate("PluginSpec", "\"%1\" is missing").arg(QLatin1String(
                                                                              key));
}
...
bool PluginSpecPrivate::readMetaData(const QJsonObject& pluginMetaData)
{
  ...
  if (!value.isObject()) return reportError(tr("Plugin meta data not found"));

  metaData = value.toObject();

  value = metaData.value(QLatin1String(PLUGIN_NAME));

  if (value.isUndefined()) return reportError(msgValueMissing(PLUGIN_NAME));

  if (!value.isString()) return reportError(msgValueIsNotAString(PLUGIN_NAME));

  name = value.toString();

  value = metaData.value(QLatin1String(PLUGIN_VERSION));

  if (value.isUndefined()) return reportError(msgValueMissing(PLUGIN_VERSION));

  if (!value.isString()) return reportError(msgValueIsNotAString(PLUGIN_VERSION));

  version = value.toString();

  if (!isValidVersion(version)) return reportError(msgInvalidFormat(PLUGIN_VERSION,
                                                                    version));

  value = metaData.value(QLatin1String(PLUGIN_COMPATVERSION));

  if (!value.isUndefined() && !value.isString()) return reportError(msgValueIsNotAString(
                                                                      PLUGIN_COMPATVERSION));

  compatVersion = value.toString(version);

  if (!value.isUndefined() && !isValidVersion(compatVersion)) return reportError(msgInvalidFormat(
                                                                                   PLUGIN_COMPATVERSION,
                                                                                   compatVersion));

  value = metaData.value(QLatin1String(PLUGIN_REQUIRED));

  if (!value.isUndefined() && !value.isBool()) return reportError(msgValueIsNotABool(
                                                                    PLUGIN_REQUIRED));
  ...
  if (experimental) enabledByDefault = false;
  ...
  if (!value.isUndefined() &&
      !Utils::readMultiLineString(value, &description)) return reportError(msgValueIsNotAString(
                                                                             DESCRIPTION));
  ...
  if (!value.isUndefined() &&
      !Utils::readMultiLineString(value, &license)) return reportError(msgValueIsNotAMultilineString(
                                                                         LICENSE));
  ...
  if (!platformSpec.isEmpty()) {
    ...
    if (!platformSpecification.isValid()) return reportError(tr(
  ...
  if (!value.isUndefined() && !value.isArray()) return reportError(msgValueIsNotAObjectArray(
                                                                     DEPENDENCIES));

  if (!value.isUndefined()) {
    QJsonArray array = value.toArray();
    foreach(const QJsonValue& v, array) {
      if (!v.isObject()) return reportError(msgValueIsNotAObjectArray(
                                              DEPENDENCIES));
      ...
      if (value.isUndefined()) return reportError(tr("Dependency: %1").arg(
                                                    msgValueMissing(
                                                      DEPENDENCY_NAME)));

      if (!value.isString()) return reportError(tr("Dependency: %1").arg(
                                                  msgValueIsNotAString(
                                                    DEPENDENCY_NAME)));
      ...
      if (!value.isUndefined() &&
          !value.isString()) return reportError(tr("Dependency: %1").arg(
                                                  msgValueIsNotAString(
                                                    DEPENDENCY_VERSION)));

      dep.version = value.toString();

      if (!isValidVersion(dep.version)) return reportError(tr(
                                                             "Dependency: %1").arg(
                                                             msgInvalidFormat(
                                                               DEPENDENCY_VERSION,
                                                               dep
  ...
  if (!value.isUndefined()) {
    QJsonArray array = value.toArray();
    foreach(const QJsonValue& v, array) {
      if (!v.isObject()) return reportError(msgValueIsNotAObjectArray(ARGUMENTS));

      QJsonObject argumentObject = v.toObject();
      PluginArgumentDescription arg;
      value = argumentObject.value(QLatin1String(ARGUMENT_NAME));

      if (value.isUndefined()) return reportError(tr("Argument: %1").arg(
                                                    msgValueMissing(
                                                      ARGUMENT_NAME)));

      if (!value.isString()) return reportError(tr("Argument: %1").arg(
                                                  msgValueIsNotAString(
                                                    ARGUMENT_NAME)));

      arg.name = value.toString();

      if (arg.name.isEmpty()) return reportError(tr(
                                                   "Argument: \"%1\" is empty").arg(
                                                   QLatin1String(
                                                     ARGUMENT_NAME)));
      ...
      if (!value.isUndefined() &&
          !value.isString()) return reportError(tr("Argument: %1").arg(
                                                  msgValueIsNotAString(
      ...
      qCDebug(pluginLog) << "Argument:" << arg.name << "Parameter:" <<
        arg.parameter
...
{
  if (QString::compare(pluginName, name, Qt::CaseInsensitive) != 0) return false;
...
{
  static const QRegExp reg(QLatin1String(
                             "([0-9]+)(?:[.]([0-9]+))?(?:[.]([0-9]+))?(?:_([0-9]+))?"));
                                      ...
                                      const QString& version2)
{
  ...
  if (!reg1.exactMatch(version1)) return 0;
...
bool PluginSpecPrivate::resolveDependencies(const QVector<PluginSpec *>& specs)
{
  ...
  if (state == PluginSpec::Resolved) state = PluginSpec::Read;  // Go back, so
                                                                // we just
                                                                // re-resolve
                                                                // the
                                                                // dependencies.
  ...
  foreach(const PluginDependency& dependency, dependencies) {
    // 查找到满足条件的插件
    PluginSpec *const found =
      Utils::findOrDefault(specs, [&dependency](PluginSpec *spec) {
    ...
    if (!found) {
      if (dependency.type == PluginDependency::Required) {
        ...
        if (!errorString.isEmpty()) errorString.append(QLatin1Char('\n'));
  ...
  bool enableTestDependencies)
{
  ...
  for (auto it = dependencySpecs.cbegin(),
       end = dependencySpecs.cend(); it != end;
       ++it) {
    if ((it.key().type != PluginDependency::Required)
        && (!enableTestDependencies ||
            (it.key().type != PluginDependency::Test))) continue;
...
bool PluginSpecPrivate::loadLibrary()
{
  ...
  if (state != PluginSpec::Resolved) {
    if (state == PluginSpec::Loaded) return true;
...
bool PluginSpecPrivate::initializePlugin()
{
  ...
  if (state != PluginSpec::Loaded) {
    if (state == PluginSpec::Initialized) return true;
...
bool PluginSpecPrivate::initializeExtensions()
{
  ...
  if (state != PluginSpec::Initialized) {
    if (state == PluginSpec::Running) return true;
...
bool PluginSpecPrivate::delayedInitialize()
{
  ...
  if (state != PluginSpec::Running) return false;
...
{
  if (!plugin) return IPlugin::SynchronousShutdown;
